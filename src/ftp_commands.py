...
class FTPCommands:
    ...
    def cmd_ls(self, args):
        ...
        try:
            ...
            if response.is_positive_preliminary():
                ...
                listing_data = NetworkUtils.receive_data_with_timeout(data_socket)
                data_socket.close()
    ...
    def cmd_mget(self, args):
        ...
        for pattern in args:
            try:
                # Get file list matching pattern (simplified)
                print(f"Downloading files matching: {pattern}")
                
                if self.client.prompt_mode:
                    confirm = input(f"Download {pattern}? (y/n): ")
                    if confirm.lower() != 'y':
                        continue
                
                # For simplicity, treat pattern as exact filename
                # In real implementation, you'd get directory listing first
                self.cmd_get([pattern])
                
            except Exception as e:
                print(f"✗ MGET error for {pattern}: {e}")
